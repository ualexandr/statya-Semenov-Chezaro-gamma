...
\begin{equation}\label{crit_ac0}
	...
	.
\end{equation}
Очевидно, что из \eqref{crit_ac0} следует \eqref{crit_pos_ac0} (например, $m_0 = 1$),
что даёт необходимость \eqref{crit_pos_ac0}.

\paragraph{Достаточность.}
Пусть верно \eqref{crit_pos_ac0}.
Покажем, что верно \eqref{crit_ac0}.
Зафиксируем $A_1$.
...
Т.к. $n\geq n_1 = 2A_0(n_0+m_0)\|x\| > n_0$, то $n>n_0$.
