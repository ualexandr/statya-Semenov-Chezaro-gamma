...
Ограниченная последовательность называется почти сходящейся,
...
Положим $\varepsilon = 1/(Q+1)$ и отыщем $n_0$.
...
Используя \eqref{lim_M(j)/j_infty_def} и положив $C=2B$,
...
\begin{equation}\label{ac0_lambda_Lorencz_neg}
	...
	.
\end{equation}
Найдём такое $\varepsilon_0$. Положим
$
...
Например, для $x_k = 1$ и $\lambda = 2$ имеем $A_\lambda x = 0 \in ac$,
но $x\notin ac$.
\end{remark}
...
Так как $x\in ac$, то в силу \eqref{eq:intro_ac} существует такое число $t$,
