...
Обозначим через $n(\lambda,i)$ возрастающую последовательность
индексов таких элементов $x$, что $x_k \geq \lambda$ тогда и только тогда,
когда $k=n(\lambda,i)$ для некоторого $i$.
Обозначим
\begin{equation}
...
Рассмотрим на $\ell_\infty$ функцию, определяемую как
\begin{equation}
...
\begin{proof}
	...
	$
		...
		[\|x-y_\varepsilon\| < \beta + \varepsilon]
		.
	$
...
Нетрудно заметить, что последовательность $\alpha(T^s x)$ монотонно убывает по $s$,
