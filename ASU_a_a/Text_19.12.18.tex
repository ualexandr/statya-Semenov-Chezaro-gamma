...
\usepackage[backend=bibtex,sorting=none]{biblatex}%,style=gost-numeric
...
\begin{proof}
	...
	Следовательно, по теореме Каратеодори~\cite[Theorem 1.53]{klenke2013probability}, для любого борелевского множества $A\subset [0,1]$ верно
	$\mes \, QA = \mes \, A$.
\end{proof}
...
\begin{proof}
	...
	(таким подмножествам также приписывается мера нуль)~\cite[Example 1.71]{klenke2013probability}.
...
в том смысле, что в левой части формулы \eqref{2.4} возможно замена $\liminf$ на $\lim$. Действительно, для любой строго возрастающей последовательности натуральных чисел $n_i$ рассмотрим последовательность
\begin{equation*}
\label{eq:definition_M_j}
M(j) = \liminf_{i\to\infty} n_{i+j} - n_i,
\end{equation*}
введенную в работе \cite{Avdeev2019}. Легко видеть, что последовательность $\{M(j)\}_{j=1}^\infty$ вогнута. Однако, в работе \cite{Fekete} (см. также \cite[I, Задача 98]{Polya72problems}) доказано, что в таком случае предел выражения $M(j)/j$ существует и справедливо равенство
...
$$
	...
	\begin{cases}
		0, & \mbox{~если~} x=y, \\
		1, & \mbox{~если~} x_1\neq y_1, \\
		2^{-k} & \mbox{в остальных случаях}, %x,y\in\{0,1\}^\N, \\
	\end{cases}
$$
...
\int_\Omega B(f_n(\omega)) dP(\omega) = B\left(\int_\Omega f_n(\omega) dP(\omega)\right).
 ...
 В работе \cite{Bjorklund} было показано множество $\mathfrak M$ является подмножеством факторизованных банаховых пределов, т.е. функционалов вида
